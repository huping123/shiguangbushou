...
class Goods extends Model
{
	...
	public function one()
	 {
	 	return Db::name('goods')->where('parid',5)->limit(1,8)->select();
 	...
 	//获取传过来的id获取单个商品信息
	  ...
	  {
	  	return Db::name('goods_color')->where("gcol_id = $id")->select();
	  }
	//    //获取传过来的id获取单个商品规格
	  public function siz($id)
	  {
	  	return Db::name('size')->where("siz_id = $id")->select();
	  }
	  //根据商品名称查询商品的id
	  public function gid($ttie)
	  {	
	  	// var_dump($ttie);
	  	 $ttie = Db::name('goods')->where("name", $ttie)->select();
	  	 return $ttie = $ttie[0]['pid'];
	  }
	//   //查询商品的id
	  public function ming($ttie)
	  {
	  	 $ming = Db::name('goods')->where("name", $ttie)->select();
	  ...
	  {
	  	return Db::name('goods')->where('name','like',"%$text%")->select();
